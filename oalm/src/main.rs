...
fn main() {
    let app = clap::App::new("Arduino Library Manager")
        ...
        .arg(clap::Arg::with_name("path").long("path")
        ...
        .subcommand(
            clap::SubCommand::with_name("new").about("Create a new library project for Arduino.")
    ...
    match app.subcommand() {
        ("install", Some(matches)) => { 
            ...
            match install::determine_arg(library_path) {
                        ...
                        Ok(http_data) => {
                            ...
                            match install::zip_file(http_data) {
                                Ok(path_to_installation) => {
                                    let display_path = path_to_installation
                                        .into_string().unwrap(); 
                ...
                Some(install::Arg::IsGitRepo) => {
                    match install::git_repository(library_path) {
                        Ok(repo_location) => {
                            println!("Successfully installed the repository project at {:?}", repo_location); 
                        }
        ...
        ("remove", Some(matches)) => match libraries::remove(matches.value_of("library").unwrap()) {
        ...
        ("new", Some(matches)) => {
            ...
            match libraries::create(project_name) {
                Ok(installation_path) => {
                    println!("Successfully created {:?} at {:?}", project_name, installation_path);
                }
                
                Err(why) => {  
                    eprintln!("{}", format!("Can not created {:?} because : {}", project_name, why));
                    process::exit(1);
                }
            } 
        }
